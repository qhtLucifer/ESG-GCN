...
from model.modules import import_class, bn_init, EncodingBlock,SAM, MambaBlock


class InfoGCN(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, in_channels=3,
                 drop_out=0, num_head=3, noise_ratio=0.1, gain=1):
        ...
        self.z_prior = torch.empty(num_class, base_channel*2)
        self.A_vector = self.get_A(graph).float()
        self.gain = gain
        ...
        # self.l1 = MambaBlock(base_channel, base_channel*2,A)
        # self.l2 = MambaBlock(base_channel, base_channel,A)
        # self.l3 = MambaBlock(base_channel, base_channel,A)
        # self.l4 = MambaBlock(base_channel, base_channel*2, A, stride=2)
        self.l5 = MambaBlock(base_channel*2)
        self.l6 = MambaBlock(base_channel*2)
        self.l7 = MambaBlock(base_channel*2)
        self.l8 = MambaBlock(base_channel*2)
        self.l9 = MambaBlock(base_channel*2)
        ...
        self.decoder = nn.Linear(base_channel*2, num_class)
    ...
    def get_A(self, graph):
        Graph = import_class(graph)()
        A_outward = Graph.A_outward_binary
        I = np.eye(Graph.num_node)
        return  torch.from_numpy(I - A_outward)
