...
class Processor():
    ...
    def load_data(self):
        ...
        data_path = f'/hy-tmp/{self.arg.dataset}/{self.arg.datacase}_aligned.npz'
    ...
    def train(self, epoch, save_model=False):
        ...
        for data, y, index in tqdm(self.data_loader['train'], dynamic_ncols=True):
            ...
            feat_wandb = {"loss_value":np.mean(loss_value), "mmd_loss_value":np.mean(mmd_loss_value), "l2_z_mean_value":np.mean(l2_z_mean_value)}
        ...
        feat_wandb.update({'train_accuracy': np.mean(acc_value)})
        wandb.log(feat_wandb)
        if save_model:
        ...
        for ln in loader_name:
            ...
            if accuracy > self.best_acc:
                ...
                with open(f'{self.arg.work_dir}/best_score.pkl', 'wb') as f:
                    pickle.dump(score_dict, f)
            total_k.update({'Accuracy ':accuracy})
            wandb.log(total_k)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            # test the best model
            path =  glob.glob(os.path.join(self.arg.work_dir, 'runs-'+str(self.arg.num_epoch)+'*'))
            print(os.path.join(self.arg.work_dir, 'runs-'+str(self.arg.num_epoch)+'*'))
